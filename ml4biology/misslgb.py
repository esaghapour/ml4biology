# -*- coding: utf-8 -*-
"""
Created on Fri Oct 23 19:25:36 2020
...
class Misslgb(BaseEstimator, TransformerMixin):
    """Missing value imputation using lightgbm.
    ...
    * [3] https://scikit-learn.org/stable/modules/generated/sklearn.ensemble.
    ...
    def __init__(self, max_iter=10, decreasing=False, missing_values=np.nan,
                 copy=True, n_estimators=100,
                 min_child_weight= 0.03,feature_fraction= 0.4,bagging_fraction= 0.4,
                 max_depth= -1,learning_rate= 0.1,bagging_seed = 11,verbosity= -1,
                 reg_alpha= 0,reg_lambda= 0,random_state= 47,min_data_in_leaf=5,n_jobs=-1
                 ):
    ...
    def _miss_lgbm(self, Ximp, mask):
        """The misslgbm algorithm"""
        ...
        while (
            ...
            for s in misscount_idx:
                ...
                if self.cat_vars_ is not None and s in self.cat_vars_:
                    ...
                    d_train = lgb.Dataset(xobs, yobs)
                    
                    clf = lgb.train(param_classifier,d_train,self.n_estimators)
                    
                    # 7. predict ymis(s) using xmis(x)
                    ymis = clf.predict(xmis)
                    del clf
                    ...
                    clf = lgb.train(param_regression,d_train,self.n_estimators)
